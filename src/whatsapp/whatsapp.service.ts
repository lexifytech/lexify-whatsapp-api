...
export class WhatsappService implements OnModuleInit {
  ...
  async createSession(sessionId: string): Promise<WASocket> {
    ...
    this.logger.log(
      `Criando sessão ${sessionId} com Baileys v${version.join('.')}`,
    );
    ...
    sock.ev.on('connection.update', (update: Partial<ConnectionState>) => {
      ...
      if (connection === 'open') {
        this.logger.log(
          `Sessão ${sessionId} conectada com sucesso ao WhatsApp Web!`,
        );
        ...
        const shouldReconnect =
          statusCode !== DisconnectReason.loggedOut && !isAuthError;

        this.logger.log(`Conexão fechada para sessão ${sessionId}:`);
        this.logger.log(
          `Status: ${statusCode}, Dados: ${JSON.stringify(errorData)}`,
        );
        ...
        if (isAuthError) {
          const sessionDir = path.join(this.SESSION_DIR, sessionId);
          this.logger.log(
            `Limpando diretório da sessão devido a erro de autenticação: ${sessionDir}`,
          );
          try {
            if (fs.existsSync(sessionDir)) {
              fs.rmSync(sessionDir, { recursive: true, force: true });
              this.logger.log(
                `Diretório da sessão ${sessionId} removido com sucesso`,
              );
            }
          } catch (err) {
  ...
  async sendMessage(sessionId: string, to: string, text: string) {
    ...
    // Envia a mensagem
    const result = await this.sessions[sessionId].sendMessage(recipient, {
      text,
    });
